...
export async function laySach(input: any): Promise<BasicReturn>{
    ...
    let sql = `
        select s.*, ks.TEN_KE_SACH, pd.TEN_PHONG_DOC
        from ${TABLE_NAME.SACH} as s
        left join ${TABLE_NAME.KE_SACH} as ks on s.KE_SACH_ID = ks.ID
        left join ${TABLE_NAME.PHONG_DOC} as pd on ks.PHONG_DOC_ID = pd.ID
        where !s.KHOA and s.TRANG_THAI
        ${expandCondition} 
        order by s.ID;
    `
    console.log(sql)
